...
void AmpForge::_UnserializeApplyConfig(nlohmann::json& config)
{
  // Pedal paths (only present in v0.1.0+)
  if (config.contains("Pedal1Path"))
  {
    mPedal1Path.Set(static_cast<std::string>(config["Pedal1Path"]).c_str());
    if (mPedal1Path.GetLength())
      _StagePedal1(mPedal1Path);
  }
  if (config.contains("Pedal2Path"))
  {
    mPedal2Path.Set(static_cast<std::string>(config["Pedal2Path"]).c_str());
    if (mPedal2Path.GetLength())
      _StagePedal2(mPedal2Path);
  }
...
// v0.1.0 (AmpForge)
// Format: namPath, irPath, pedal1Path, pedal2Path, then all params via SerializeParams order

int _GetConfigFrom_0_1_0(const iplug::IByteChunk& chunk, int startPos, nlohmann::json& config)
{
  int pos = startPos;
  WDL_String path;
  pos = chunk.GetStr(path, pos);
  ...
  config["IRPath"] = std::string(path.Get());
  pos = chunk.GetStr(path, pos);
  config["Pedal1Path"] = std::string(path.Get());
  pos = chunk.GetStr(path, pos);
  config["Pedal2Path"] = std::string(path.Get());

  // All params in EParams order (stored as doubles by SerializeParams)
  static const std::vector<std::string> kParamNames {
    ...
    "CalibrateInput", "InputCalibrationLevel", "OutputMode", "Slim",
    "DoublerActive", "DoublerDelay", "Transpose", "InputModeStereo",
    "EQ 65Hz", "EQ 125Hz", "EQ 250Hz", "EQ 500Hz", "EQ 1kHz",
    "EQ 2kHz", "EQ 4kHz", "EQ 8kHz", "EQ 16kHz",
    "HPF Freq", "LPF Freq", "EQPageActive",
    "TunerLive", "TunerRef"
  };
...
int AmpForge::_UnserializeStateWithKnownVersion(const iplug::IByteChunk& chunk, int startPos)
{
  ...
  // AmpForge 0.1.x – version range 0.1.0 <= v < 0.7.0
  if (version >= _Version(0, 1, 0) && !( version >= _Version(0, 7, 0) ))
  {
    pos = _GetConfigFrom_0_1_0(chunk, pos, config);
  }
...
int AmpForge::_UnserializeStateWithUnknownVersion(const iplug::IByteChunk& chunk, int startPos)
{
  nlohmann::json config;
